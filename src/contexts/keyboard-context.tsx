...
export function KeyboardProvider({ children }: { children: React.ReactNode }) {
    ...
    const [layout, setLayout] = useState<"default" | "numeric" | "accent">("default");
    ...
    useEffect(() => {
        const handleFocusIn = (e: Event) => {
            ...
            if (
                ...
                const isNumeric = type === "number" || type === "tel" || inputMode === "numeric" || dataLayout === "numeric";
    ...
    const openKeyboard = React.useCallback((input: HTMLInputElement | HTMLTextAreaElement) => {
    ...
    const handleKeyPress = React.useCallback((key: string) => {
        ...
        if (key === "{bksp}") {
            ...
            if (start === end) {
                if (start > 0) {
                    nextVal = currentVal.substring(0, start - 1) + currentVal.substring(end);
                    nextCursor = start - 1;
                }
            } else {
        ...
        if (!isSpecialAction) {
            ...
            if (charToInsert.length === 1 && /[a-z\u00C0-\u00FF]/i.test(charToInsert)) {
                const wantUpper = isCaps !== isShift; // XOR logic for Shift vs Caps
                finalKey = wantUpper ? charToInsert.toUpperCase() : charToInsert.toLowerCase();
            }

            const nextVal = currentVal.substring(0, start) + finalKey + currentVal.substring(end);
    ...
    const updateInputValue = (input: HTMLInputElement | HTMLTextAreaElement, nextVal: string, nextCursor: number) => {
        ...
        requestAnimationFrame(() => {
            try {
                ...
                if (isSelectionSupported && typeof input.setSelectionRange === 'function') {
