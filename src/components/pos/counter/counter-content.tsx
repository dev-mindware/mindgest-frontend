...
import { CategorySection, ProductSection, CartSection, ProductMock } from "./counter-comps";
import { BarcodeProductModal, MODAL_BARCODE_PRODUCT_ID } from "./counter-comps/barcode-product-modal";
import { useModal } from "@/stores/use-modal-store";
import { useGetCategories } from "@/hooks/category";
import { useGetItems } from "@/hooks/stock";
import { useQueryState } from "nuqs";
import { PosCategorySkeleton, PosProductSectionSkeleton, PosCartSkeleton } from "@/components/common/skeletons";
...
export function CounterContent() {
    ...
    const { categories: apiCategories, isLoading: isLoadingCategories } = useGetCategories();
    ...
    const [scannedProduct, setScannedProduct] = useState<ProductMock | null>(null);
        ...
        const handleGlobalKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === "Enter") {
                if (barcodeBuffer) {
                    ...
                    const product = (apiProducts as any[]).find((p) => p.barcode === barcode || p.sku === barcode);
    ...
    const onConfirmScan = (quantity: number) => {
        ...
        setCarts(prev => ({
            ...
            [activeCart]: {
                ...prev[activeCart],
                [scannedProduct.id]: (prev[activeCart][scannedProduct.id] || 0) + quantity
            }
        }));
    };
    ...
    const currentCategoryName = apiCategories.find(c => c.value === selectedCategory)?.label;
        ...
        <div className="flex h-full overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0 gap-4 p-4">
                {isLoadingCategories ? (
                    ...
                    <CategorySection
                        categories={apiCategories.map(c => ({ id: c.value, name: c.label, count: 0 }))}
                ...
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold">{currentCategoryName || "Todos"}</h2>
                </div>
            ...
            <div className="w-[450px] border-l border-border/50 backdrop-blur-sm">
                <Tabs value={activeCart} onValueChange={(v) => setActiveCart(v as CartType)} className="h-full flex flex-col">
                    <div className="px-6 pt-4 flex items-center justify-between">
                        <TabsList>
                            <TabsTrigger value="invoice" className="text-xs tracking-wider font-semibold">Factura</TabsTrigger>
                            <TabsTrigger value="proforma" className="text-xs tracking-wider font-semibold">Proforma</TabsTrigger>
                        </TabsList>
                    </div>
