              ...
              if (card.modalId === "pos-requests") {
                return (
                  <div
                    ...
                    title={hasPendingRequests ? `${requestsCount} pedido(s) pendente(s) — clique para ver` : "Sem pedidos pendentes"}
                  >
                    {/* Radar rings — only when there are pending requests */}
                    {hasPendingRequests && (
                      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none">
                        <span className="absolute inset-0 rounded-xl border-2 border-amber-400/60 animate-ping" style={{ animationDuration: "1.5s" }} />
                        <span className="absolute inset-0 rounded-xl border border-amber-400/30 animate-ping" style={{ animationDuration: "2s", animationDelay: "0.4s" }} />
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between items-start">
                              <h2 className={cn(
                                ...
                                hasPendingRequests ? "text-amber-500" : "text-primary"
                              )}>
                                {requestsCount}
                              </h2>
                              <div className={cn(
                                "p-2 rounded-md shrink-0 relative",
                                hasPendingRequests ? "bg-amber-400/15 text-amber-500" : "bg-muted text-muted-foreground"
                              )}>
                    ...
                    {hasPendingRequests && (
                      <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-56 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <div className="bg-popover text-popover-foreground text-xs font-medium rounded-xl border border-border/60 shadow-xl px-3 py-2 leading-relaxed text-center">
                          {requestsCount} colaborador{requestsCount !== 1 ? "es" : ""} aguarda{requestsCount === 1 ? "" : "m"} autorização para abrir caixa. Clique para ver e aprovar.
                        </div>
