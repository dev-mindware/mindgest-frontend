...
import { Card, CardContent, Button, Icon, Popover, PopoverContent, PopoverAnchor, Input, Avatar, AvatarFallback, AvatarImage, Tooltip, TooltipContent, TooltipTrigger, Badge } from "@/components";
...
export function ProductCard({ product, quantity, onAdd, onRemove, onUpdateQuantity }: ProductCardProps) {
            ...
            <CardContent className="p-3 sm:p-4 flex-1 flex flex-col gap-3">
                ...
                <div className="flex gap-3 sm:gap-3.5">
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <div className="flex flex-col justify-between py-0.5 sm:py-1 flex-1 overflow-hidden cursor-help">
                                ...
                                <div className="pt-1.5 flex flex-wrap gap-1">
                                    {product.reserved > 0 ? (
                                        <Badge variant="success" className="text-[9px] px-1.5 py-0 border-none bg-green-500/10 text-green-600">
                                            <div className="w-1 h-1 rounded-full bg-green-500 mr-1" />
                                            {product.reserved} em estoque
                                        </Badge>
                                    ) : (
                                        <Badge variant="destructive" className="text-[9px] px-1.5 py-0 border-none bg-destructive/10 text-destructive">
                        ...
                        <TooltipContent side="right" align="start" className="max-w-[220px] p-3 flex flex-col gap-1.5 shadow-xl border-border/50 z-[100]">
                ...
                <div className="flex items-center justify-between mt-auto pt-3 border-t border-border/40">
                    <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider scale-75 origin-left">Preço</span>
                                ...
                                {quantity === 0 ? (
                                    ...
                                    <div className="flex items-center gap-1 bg-muted/50 rounded-full p-0.5 sm:p-1 border border-border/60 shadow-sm">
                                        <Button
                                            ...
                                            onClick={onRemove}
                                        >
                                            <Icon name="Minus" className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                                        </Button>
