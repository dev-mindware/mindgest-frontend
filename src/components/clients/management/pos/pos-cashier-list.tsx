...
import { useCurrentCashierStore, useModal } from "@/stores";
...
export function PosCashierList({ cashiers, isLoading, viewMode }: PosCashierListProps) {
    ...
    const columns: Column<Cashier>[] = [
            ...
            render: (_, item) => (
                <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 rounded-lg overflow-hidden bg-muted/30 border border-muted-foreground/5 shrink-0">
                        <Image src="/pos-computer-icon.png" alt="POS" fill className="object-contain p-1" />
                    </div>
                    <div>
        ...
        },
        {
            ...
            render: (_, item) => <ButtonOnlyAction data={item} actions={cashierActions} />
        }
    ];
        ...
        return (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {cashiers.map((cashier) => (
                    <Card key={cashier.id} className="group hover:border-primary/40 transition-all duration-300 shadow-none border-muted-foreground/10 overflow-hidden">
                        <CardContent className="p-5 space-y-6">
                            <div className="flex items-start justify-between">
                                ...
                                <ButtonOnlyAction data={cashier} actions={cashierActions} />
                            </div>

                            <div className="space-y-4">
                                <div className="space-y-1">
                                    <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-widest">Responsável</p>
                                    <p className="text-sm truncate">{cashier.user}</p>
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-widest">Total</p>
                                        <p className="text-lg tracking-tight">{formatCurrency(cashier.totalSold).split(",")[0]}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-widest">Tempo</p>
                                        <p className="text-lg tracking-tight">{cashier.activityTime}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-end">
                                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Progresso</p>
                                    <span className="text-xs text-primary">{cashier.progress}%</span>
                                </div>
                                <Progress value={cashier.progress} className="h-1.5 bg-muted rounded-full" />
                            </div>
                        </CardContent>
                    </Card>
                ))}
