...
import { posOpeningSchema, PosOpeningFormData } from "@/schemas";
...
export function PosOpeningModal() {
        ...
        return (
            <GlobalModal id="opening-cashier" title="Abertura de Caixa" canClose>
                <div className="p-8 text-center flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground font-medium">Carregando lojas...</p>
                </div>
            </GlobalModal>
        );
    }
    ...
    return (
        <GlobalModal
            ...
            description={isEdit ? "Edite as configurações da sessão deste caixa" : "Faça a abertura de caixa aqui e controle o fluxo de vendas dos seu funcionários"}
            canClose
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <Label className="text-sm font-medium">
                            {isEdit ? "Caixa sendo editado" : "Caixas Disponíveis para Abertura"}
                        </Label>
                            ...
                            <PopoverTrigger asChild disabled={isEdit}>
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        selectedCashierIds.length > 0 && "text-foreground font-medium",
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {isEdit ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Badge variant="outline" className="text-[10px] py-0">{currentCashier?.user}</Badge>
                                            </div>
                                        ) : selectedCashierIds.length > 0 ? (
                                            <div className="flex items-center gap-2">
                                                <span>{selectedCashierIds.length} caixas selecionados</span>
                                            </div>
                                        ) : (
                                            <span>Escolha os caixas</span>
                                        )}
                                    </div>
                                    {!isEdit && <Icon name="ChevronDown" className="ml-2 h-4 w-4 shrink-0 opacity-50" />}
                                </Button>
                            </PopoverTrigger>
                                                ...
                                                {availableCashiers.map((cashier) => (
                                                    <div
                                                        ...
                                                        className={cn(
                                                            ...
                                                            field.value?.includes(cashier.id) && "bg-primary/5 border-primary/10"
                                                        )}
                                                        onClick={() => {
                                                    ...
                                                    >
                                                        ...
                                                        <div className="grid gap-0.5 leading-none">
                                                            ...
                                                            <p className="text-[10px] text-green-500">{cashier.status}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </>
                        ...
                        {errors.cashierIds && (
                            <p className="text-[10px] font-bold text-destructive uppercase tracking-widest">{errors.cashierIds.message}</p>
                        )}
                    </div>
                </div>
