...
import { StockUnreserveFormData, stockUnreserveSchema } from "@/schemas/stock-schema";
...
export function UnreserveStockModal() {
    ...
    async function onSubmit(data: StockUnreserveFormData) {
        ...
        if (data.amount > currentStock.reserved) {
            ErrorMessage(`A quantidade não pode ser maior que ${currentStock.reserved} (reservado)`);
            return;
        }
        ...
        >
            <div className="mb-4 p-4 bg-muted rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                    ...
                    <span className="text-orange-600 font-semibold">{currentStock.reserved}</span>
                </p>
                <p className="text-sm text-muted-foreground">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                />
                <p className="text-xs text-muted-foreground">Máximo: {currentStock.reserved}</p>
